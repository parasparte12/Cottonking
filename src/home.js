...
const Home = () => {
  ...
  // FIXED: Better quality images that actually work
  const allProducts = [
    // MEN Products
    { 
      id: 1, 
  ...
  // ✅ FIXED ADD TO CART FUNCTION - NOW WITH CONSOLE LOGS FOR DEBUGGING
  const handleAddToCart = (e, product) => {
    ...
    try {
      ...
      if (existingIndex > -1) {
        ...
        console.log('✅ Quantity increased:', cart[existingIndex]);
        ...
        console.log('✅ New item added:', cart[cart.length - 1]);
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map(product => (
            <div key={product.id} className="product-card">
              ...
              {/* ✅ BUTTON WITH FIXED onClick HANDLER */}
              <button 
